...
const CRON_DMIDS     = `0 18 * * 0 cd "${ROOT}" && ${NODE_BIN} "${DMIDS_SCRIPT}" --rescore >> "${DMIDS_LOG}" 2>&1 ${MARKER_DMIDS}`;
const DISCOVERY_AUDIT_SCRIPT = join(ROOT, 'scripts', 'egx_discovery_audit.mjs');
const DISCOVERY_AUDIT_LOG    = join(ROOT, 'logs', 'discovery_audit.log');
...
// Regime-conditional sweep — weekly before discovery audit
const REGIME_SWEEP_SCRIPT = join(ROOT, 'scripts', 'egx_regime_conditional_sweep.mjs');
const REGIME_SWEEP_LOG    = join(ROOT, 'logs', 'regime_sweep.log');
const MARKER_REGIME_SWEEP = '# EGX-REGIME-SWEEP-W';
const CRON_REGIME_SWEEP_W = `15 18 * * 0 cd "${ROOT}" && ${NODE_BIN} "${REGIME_SWEEP_SCRIPT}" >> "${REGIME_SWEEP_LOG}" 2>&1 ${MARKER_REGIME_SWEEP}`;

// Hypothesis sandbox bridge — weekly after regime sweep
const HYP_BRIDGE_SCRIPT = join(ROOT, 'scripts', 'egx_hypothesis_sandbox_bridge.mjs');
const HYP_BRIDGE_LOG    = join(ROOT, 'logs', 'hypothesis_bridge.log');
const MARKER_HYP_BRIDGE = '# EGX-HYPOTHESIS-BRIDGE-W';
const CRON_HYP_BRIDGE_W = `20 18 * * 0 cd "${ROOT}" && ${NODE_BIN} "${HYP_BRIDGE_SCRIPT}" >> "${HYP_BRIDGE_LOG}" 2>&1 ${MARKER_HYP_BRIDGE}`;

// Discovery Fabric L11 — merge + backtest gate + ML manifest (daily + weekly full)
const FABRIC_SCRIPT = join(ROOT, 'scripts', 'egx_discovery_fabric.mjs');
const FABRIC_LOG    = join(ROOT, 'logs', 'discovery_fabric.log');
const MARKER_FABRIC_D = '# EGX-DISCOVERY-FABRIC-D';
const MARKER_FABRIC_W = '# EGX-DISCOVERY-FABRIC-W';
const CRON_FABRIC_D   = `35 11 * * 0-4 cd "${ROOT}" && ${CRON_ENV} ${NODE_BIN} "${FABRIC_SCRIPT}" >> "${FABRIC_LOG}" 2>&1 ${MARKER_FABRIC_D}`;
const CRON_FABRIC_W   = `42 18 * * 0 cd "${ROOT}" && ${CRON_ENV} ${NODE_BIN} "${FABRIC_SCRIPT}" >> "${FABRIC_LOG}" 2>&1 ${MARKER_FABRIC_W}`;
...
const GAP_REPAIR_LOG    = join(ROOT, 'logs', 'gap_repair.log');
const MARKER_GAP_REPAIR = '# EGX-GAP-REPAIR-D';
const CRON_GAP_REPAIR_D = `20 5 * * 0-4 cd "${ROOT}" && ${CRON_ENV} ${NODE_BIN} "${GAP_REPAIR_SCRIPT}" --skip-automate --skip-pine >> "${GAP_REPAIR_LOG}" 2>&1 ${MARKER_GAP_REPAIR}`;
const MARKER_GAP_REPAIR_W = '# EGX-GAP-REPAIR-W';
const CRON_GAP_REPAIR_W = `10 6 * * 0 cd "${ROOT}" && ${CRON_ENV} ${NODE_BIN} "${GAP_REPAIR_SCRIPT}" >> "${GAP_REPAIR_LOG}" 2>&1 ${MARKER_GAP_REPAIR_W}`;

// Intraday OHLCV (60min + 15min) — weekly core universe after EOD
const INTRADAY_SCRIPT = join(ROOT, 'scripts', 'fetch_egx_intraday.mjs');
...
const MARKER_INTRADAY_FULL_M = '# EGX-INTRADAY-FULL-M';
const CRON_INTRADAY_FULL_M = `30 22 1-7 * 0 cd "${ROOT}" && ${CRON_ENV} ${NODE_BIN} "${INTRADAY_SCRIPT}" --resume >> "${INTRADAY_LOG}" 2>&1 ${MARKER_INTRADAY_FULL_M}`;
...
const MARKER_PRE_SESSION   = '# EGX-PRE-SESSION-DAILY';
const MARKER_MLADV_W       = '# EGX-MLADV-WEEKLY';
const PRE_SESSION_SCRIPT   = join(ROOT, 'scripts', 'egx_pre_session.mjs');
const PRE_SESSION_LOG      = join(ROOT, 'logs', 'pre_session.log');
...
const SYSTEM_HEALTH_SCRIPT = join(ROOT, 'scripts', 'python', 'system_health_check.py');
const SYSTEM_HEALTH_LOG = join(ROOT, 'logs', 'system_health.log');
const MARKER_AUDIT_W     = '# EGX-AUDIT-WEEKLY';
const AUDIT_ORCH_SCRIPT  = join(ROOT, 'scripts', 'egx_system_audit_orchestrator.mjs');
const AUDIT_LOG          = join(ROOT, 'logs', 'system_audit.log');
...
// Pre-session bundle: 7:25 AM القاهرة (5:25 UTC) — audit + funnel + gate_simulate
const CRON_PRE_SESSION   = `25 5 * * 0-4 cd "${ROOT}" && ${CRON_ENV} ${NODE_BIN} "${PRE_SESSION_SCRIPT}" --next >> "${PRE_SESSION_LOG}" 2>&1 ${MARKER_PRE_SESSION}`;
...
const CRON_POST_SESSION  = `45 15 * * 0-4 cd "${ROOT}" && ${locked('egx-post-session', `${NODE_BIN} "${POST_SESSION_SCRIPT}"`)} >> "${POST_SESSION_LOG}" 2>&1 && ${PYTHON_BIN} "${SYSTEM_HEALTH_SCRIPT}" --quick >> "${SYSTEM_HEALTH_LOG}" 2>&1 ${MARKER_POST_SESSION}`;
// Institutional audit suite: كل أحد 7:00 AM القاهرة (5:00 UTC)
const CRON_AUDIT_W       = `0 5 * * 0 cd "${ROOT}" && ${NODE_BIN} "${AUDIT_ORCH_SCRIPT}" --all >> "${AUDIT_LOG}" 2>&1 ${MARKER_AUDIT_W}`;
...
if (SHOW) {
  ...
  if (!current.trim()) {
    ...
    const hasPreSess    = current.includes(MARKER_PRE_SESSION);
    ...
    console.log(hasPreSess   ? '✅  Pre-session bundle cron'      : '❌  Pre-session bundle مفقود');
...
if (REMOVE) {
  const current = getCurrentCron();
  const ALL_MARKERS = [MARKER, MARKER_MACRO, MARKER_TG, MARKER_DMIDS, MARKER_REGIME_SWEEP, MARKER_HYP_BRIDGE, MARKER_DISCOVERY_AUDIT, MARKER_PERPETUAL, MARKER_FABRIC_D, MARKER_FABRIC_W, MARKER_TV_MICRO, MARKER_GAP_REPAIR, MARKER_GAP_REPAIR_W, MARKER_INTRADAY_W, MARKER_INTRADAY_LIQ_W, MARKER_INTRADAY_FULL_M, MARKER_DHVD, MARKER_EVO_Q, MARKER_EVO_F, MARKER_COG_Q, MARKER_COG_F, MARKER_GRAPH_W, MARKER_RL_W, MARKER_EXPLAIN_Q, MARKER_MACRO_G, MARKER_TV_LIVE_1, MARKER_TV_LIVE_2, MARKER_INTEGRITY, MARKER_UMCG_W, MARKER_CAUSAL_W, MARKER_FAILURE_D, MARKER_EXPLOSION_D, MARKER_DNA_W, MARKER_RESEARCH_W, MARKER_EXECUTION_W, MARKER_SYNTHESIS_D, MARKER_PRIORITIZE_D, MARKER_MEMORY_W, MARKER_META_W, MARKER_PORTFOLIO_D, MARKER_TRANSITION_D, MARKER_ANTILAWS_D, MARKER_STAT_W, MARKER_ARBITRATE_D, MARKER_OBSERVATORY_D, MARKER_COMPRESSION_D, MARKER_UNCERTAINTY_W, MARKER_SANDBOX_W, MARKER_GOVERNANCE_D, MARKER_COGBUS_D, MARKER_PRESSURE_W, MARKER_EXECREAL_W, MARKER_HEALTH_D, MARKER_FUNNEL_D, MARKER_DEPS_W, MARKER_TRAIN_W, MARKER_NIGHT_W, MARKER_PROD_STATUS_D, MARKER_SESSION_READY, MARKER_PRE_SESSION, MARKER_CRON_LOG_CHK, MARKER_QUALITY_W, MARKER_LEARNING_W, MARKER_PROD_READY_W, MARKER_VERIFY_D, MARKER_POST_SESSION, MARKER_AUDIT_W, MARKER_MLADV_W];
...
const filtered = current.split('\n')
  ...
  .join('\n').trim();
const ALL_CRONS = [
  CRON_LINE, CRON_MACRO, CRON_TG, CRON_TV_LIVE_1, CRON_TV_LIVE_2, CRON_DMIDS, CRON_REGIME_SWEEP_W, CRON_HYP_BRIDGE_W, CRON_DISCOVERY_AUDIT, CRON_PERPETUAL_W, CRON_FABRIC_D, CRON_FABRIC_W, CRON_TV_MICRO, CRON_GAP_REPAIR_D, CRON_GAP_REPAIR_W, CRON_INTRADAY_W, CRON_INTRADAY_LIQ_W, CRON_INTRADAY_FULL_M, CRON_DHVD,
  ...
  CRON_HEALTH_D, CRON_FUNNEL_D, CRON_PROD_STATUS_D, CRON_SESSION_READY, CRON_PRE_SESSION, CRON_LOG_CHECK, CRON_QUALITY_W, CRON_LEARNING_W, CRON_PROD_READY_W, CRON_VERIFY_D, CRON_POST_SESSION, CRON_AUDIT_W, CRON_DEPS_W, CRON_TRAIN_W, CRON_NIGHT_W, CRON_MLADV_W
].join('\n');
const newCron  = dedupeCronLines(filtered ? `${filtered}\n${ALL_CRONS}` : ALL_CRONS);

setCron(newCron);

console.log('✅  EGX Automation مثبّت بنجاح (55 مهمة):');
...
console.log(`   ✅ Pre-session:   25 5 * * 0-4      (الأحد-الخميس 7:25 AM) → egx_pre_session.mjs --next`);
