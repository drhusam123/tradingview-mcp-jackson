...
const ALL_CRONS = [
  ...
  CRON_HEALTH_D, CRON_FUNNEL_D, CRON_PROD_STATUS_D, CRON_SESSION_READY, CRON_LOG_CHECK, CRON_QUALITY_W, CRON_PROD_READY_W, CRON_VERIFY_D, CRON_POST_SESSION, CRON_DEPS_W, CRON_TRAIN_W, CRON_NIGHT_W, CRON_MLADV_W
].join('\n');
const newCron  = filtered ? `${filtered}\n${ALL_CRONS}` : ALL_CRONS;

setCron(newCron);

console.log('✅  EGX Automation مثبّت بنجاح (52 مهمة):');
