...
function generateDataPipelineAudit(dataLayer) {
  ...
  lines.push('', '## Fixes Applied', '', '- Automated via `egx_data_layer_audit.mjs`', '');
  lines.push('## Remaining Risks', '', dataLayer?.pass ? '- None critical' : '- Review failed L0/L1 checks above', '');
...
function generateEnginesAudit() {
  const engines = [
    { name: 'LRE 4.0', cmd: 'egx:lre:status', artifact: 'lre_4_0_status_last.json' },
    { name: 'MED 0.3', cmd: 'egx:med:run', artifact: 'med_0_3_status_last.json' },
    { name: 'MDE shadow', cmd: 'egx:mde:shadow', artifact: 'mde_pilot_shadow_last.json' },
    { name: 'P6 delivered', cmd: 'egx:phase22:p6-delivered', artifact: 'phase22_p6_delivered_last.json' },
    { name: 'LRE OOS acc', cmd: 'lre_oos_accumulator.py', artifact: 'lre_oos_accumulator_last.json' },
    { name: 'Graduation', cmd: 'egx:graduation:complete', artifact: 'graduation_final_last.json' },
  ];
  ...
  const lines = [
    ...
    `**Generated:** ${new Date().toISOString()}`,
    '',
    '| Engine | Command | Output | Last Run | Status |',
    '|--------|---------|--------|----------|--------|',
  ];
  ...
  for (const e of engines) {
    ...
    lines.push(`| ${e.name} | \`${e.cmd}\` | \`${e.artifact}\` | ${runAt} | ${status} |`);
    results.push({ engine: e.name, status, artifact: e.artifact });
  }

  // Registry check
  const regPath = join(PROJECT_ROOT, 'scripts/lib/discovery_engine_registry.mjs');
  const regText = existsSync(regPath) ? readFileSync(regPath, 'utf8') : '';
  const regEngines = ['med_daily_chain', 'lre_4_0', 'mde_signal', 'causal_discovery'];
  lines.push('', '## Registry', '');
  for (const id of regEngines) {
    lines.push(`- \`${id}\`: ${regText.includes(id) ? '✅ wired' : '❌ missing'}`);
  }
