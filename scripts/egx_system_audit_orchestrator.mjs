...
function generateGatesAudit(notifAudit) {
  ...
  const lines = [
    ...
    `| gates | scored signals | gate_audit_snapshots | pass/fail+reason | ${notifAudit?.actionable?.deliverable ?? '—'} deliverable | ✅ | low — gate_doctor logs blockers |`,
    `| promotion | quality+edge pass | final_signals | actionable rows | DB=${notifAudit?.actionable?.db ?? '—'} | ✅ | low |`,
    `| formatter | actionable SSOT | final_signals.actionable | telegram messages | deliverable=${notifAudit?.actionable?.deliverable ?? '—'} | ✅ | low — diagnostics JSON |`,
    `| send gate | deliverable+pre_send | safety veto | send or block+reason | root=${notifAudit?.root_cause_category ?? notifAudit?.root_cause ?? '—'} | ✅ | medium — safety blocks logged |`,
    '',
    '## Gate doctor',
    '',
    '```json',
