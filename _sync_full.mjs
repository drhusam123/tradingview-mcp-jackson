...
for (const sym of symbols) {
  ...
  await ev(Runtime, `(function(){try{${API}.setSymbol('${tvSym}');${API}.setResolution('${timeframe}');return true;}catch(e){return String(e)}})()`);
  ...
  if (pull && pull.bars && pull.bars.length > 0) {
    results[sym] = { tvSym: pull.sym, res: pull.res, total: pull.total, bars: pull.bars };
    ok++;
    process.stderr.write(`✓ ${sym} (${pull.bars.length} bars, last close ${pull.bars[pull.bars.length-1].close})\n`);
  } else {
    results[sym] = { error: (pull && pull.__err) || 'empty' };
    fail++;
    process.stderr.write(`✗ ${sym} — ${(pull && pull.__err) || 'empty'}\n`);
  }
}
await client.close();
